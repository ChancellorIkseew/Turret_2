...
void Schematic::drawBlockFrame(Renderer& renderer, const TileCoord tile, const int size, const uint32_t color) {
    ...
    //
    renderer.drawRect(position, PixelCoord(10, 4), PixelCoord(0, 0), 0.f, color);
    renderer.drawRect(position, PixelCoord(4, 10), PixelCoord(0, 0), 0.f, color);
    //
    const PixelCoord topRight(position.x + pixelSize, position.y);
    renderer.drawRect(topRight, PixelCoord(10, 4), PixelCoord(10, 0), 0.f, color);
    renderer.drawRect(topRight, PixelCoord(4, 10), PixelCoord(4, 0), 0.f, color);
    //
    const PixelCoord bottomLeft(position.x, position.y + pixelSize);
    renderer.drawRect(bottomLeft, PixelCoord(10, 4), PixelCoord(0, 4), 0.f, color);
    renderer.drawRect(bottomLeft, PixelCoord(4, 10), PixelCoord(0, 10), 0.f, color);
    //
    const PixelCoord bottomRight(position.x + pixelSize, position.y + pixelSize);
    renderer.drawRect(bottomRight, PixelCoord(10, 4), PixelCoord(10, 4), 0.f, color);
    renderer.drawRect(bottomRight, PixelCoord(4, 10), PixelCoord(4, 10), 0.f, color);
}
...
void Schematic::drawGhosts(Renderer& renderer, const Presets& presets, const uint64_t timeMs) const {
    ...
    for (const auto& blueprint : blueprints) {
        if (blueprint.action == BPAction::build)
            ...
            drawBlockFrame(renderer, blueprint.tile, blueprint.size, cl::RED);
    }
}
