...
Texture2D::Texture2D(int width, int height, const unsigned char* data) {
    ...
    glTextureStorage2D(textureID, MIPMAP_LEVELS_COUNT, GL_RGBA8, width, height);
    //
    constexpr GLint MIPMAP_LEVEL = 0;
    constexpr GLint OFFSET_X = 0;
    constexpr GLint OFFSET_Y = 0;
    glTextureSubImage2D(textureID, MIPMAP_LEVEL, OFFSET_X, OFFSET_Y, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
    //
