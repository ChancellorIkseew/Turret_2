...
void SoundQueue::play(Audio& audio, const Camera& camera, const int64_t timeMs) {
    ...
    for (const auto& [name, agg] : oneShots) {
        if (agg.count == 0)
            continue;
        const PixelCoord averagePos = agg.accumulatedPosition / static_cast<float>(agg.count);
        ...
        const float countGainBoost = std::min(std::sqrt(static_cast<float>(agg.count)), 1.5f);
        const float finalGain = (1.0f + volumeOffset) * countGainBoost;
        audio.playDiegetic(name, averagePos, camera, finalGain, finalPitch);
    }
